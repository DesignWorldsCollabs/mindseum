...
module.exports.setup = function setup(app, config, cb) {
    ...
    .head('/summarize', function checkUrl(req, res, next) {
      ...
      function onResponse(err, response) {
        ...
        if (((response || {}).header || {}).location) {
          ...
          var location = encodeURIComponent(protocol + '//' + host + redirect.path)
          res.location((process.env.PROTOCOL || req.protocol) + '://' + req.get('host') + '/summarize?url=' + location);
        }
  ...
  var publicPath = config.get('public')
  if (config.get('temp').length) publicPath = config.get('temp')
  app.writeScripts(store, publicPath, {}, function onWriteScripts(err) {
    cb(err, expressApp);
  });
}
