...
module.exports.setup = function setup(app, config, cb) {
  ...
  expressApp
    ...
    .get('/summarize', function summarizeUrl(req, res, next) {
      ...
      var endpoint = 'http://clipped.me/algorithm/clippedapi.php?url=' + url
      request.get(endpoint).type('json').timeout(5000).end(onResponse)
      function onResponse(err, response) {
        if (err) return res.status(500).json({error: 'request timeout'})
        var json
        try {
          json = JSON.parse(response.text)
          res.status(response.status)
        } catch (e) {
          json = {error: e, text: response.text}
          res.status(500)
        }
        res.json(json)
      }
